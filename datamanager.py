...
class DataManager(object):
    ...
    async def download_train_data(self, training_data):
        ...
        logging.debug("model_sha1={m}, train_dir={t}, validate_dir={v}".format(m=model_sha1,t=train_dir,v=validate_dir))
        ...
        try:
            ...
            logging.error('Cant train model by data from csv {csv_url}, random_seed {r}: {error}'.format(csv_url=training_data['csv']['url'], r=training_data['metadata']['random_seed'], error=error))
            logging.error(exc)
        ...
        if train_size < self.MIN_TRAIN_SIZE or validate_size < self.MIN_VALIDATE_SIZE:
            ...
            logging.error('Cant train model by data from csv {csv_url}: {error}'.format(csv_url=training_data['csv']['url'], error=error))
            return None, None, None, None, error
